...
const AdminPendingModal = ({ onClose }) => {
...
// Delete pending image (using correct pending endpoint)
const handleDeleteFile = async (pendingId) => {
if (!window.confirm('Delete this pending image?')) return;
try {
await galleryService.deletePendingImage(pendingId); // <-- correct endpoint
setPendingFiles(p => p.filter(i => i._id !== pendingId));
setSuccess('Pending image deleted.');
setTimeout(() => setSuccess(null), 2500);
} catch (err) {
console.error(err);
...
return ( <div className="modal-backdrop"> <div className="modal large card-modal"> <div className="card-header"> <h3>Pending Gallery Approvals</h3> <button className="close" onClick={onClose}>×</button> </div>
              ...
              {pendingFiles.map(item => (
                <div key={item._id} className="pending-item">
                  <img
                    ...
                    className="thumb"
                    onError={e => e.target.src = 'https://via.placeholder.com/150x100?text=Not+Found'}
                  />
                  <div className="meta">
                    ...
                    <div className="actions">
                      ...
                      <button
                        className="btn-action"
                        onClick={() => handleDeleteFile(item._id)} // <-- no albumId here
                      >
                        Delete
                      </button>
                    </div>
