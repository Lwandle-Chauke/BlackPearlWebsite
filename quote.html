...
<head>
  ...
  <title>Get a Quote | Black Pearl Coach Charters and Tours</title>
...
<!-- Sign In / Register Popup Overlay -->
  ...
  <main class="quote-page">
    <section class="quote-section" style="margin-top: 60px;">
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
      ...
      if (vehicleType) {
        ...
        if (vehicleSelect) {
          // Try to find the option that matches the vehicle type
          for (let i = 0; i < vehicleSelect.options.length; i++) {
            if (vehicleSelect.options[i].text === vehicleType) {
              vehicleSelect.selectedIndex = i;
              
              // Show a success message to indicate the vehicle was pre-selected
      ...
      if (quoteForm) {
        quoteForm.addEventListener('submit', function(e) {
          ...
          if (isValid) {
            ...
            alert('Thank you for your quote request! We will contact you shortly with your customized quote.');
            quoteForm.reset();
            
            // Reset the vehicle type if it was pre-selected
            const vehicleSelect = document.getElementById('vehicleType');
            if (vehicleSelect) {
